...
async function mockGetTickets(tickets, paginationOverrides = {}) {
...
describe("E2E-01 — Desktop view: search, filter, and pagination", () => {
    it("renders the ticket table on desktop and shows ticket data", async () => {
        render(_jsx(MyTicketsPage, { categories: CATEGORIES, onNewTicket: vi.fn() }));
    ...
    it("E2E-01a — typing a search term and submitting calls getTickets with the search param", async () => {
        ...
        await waitFor(() => {
            expect(mockFn).toHaveBeenCalledWith(expect.objectContaining({ search: "cracked screen" }));
        });
    });
    it("E2E-01b — selecting a priority filter calls getTickets with the priority param", async () => {
        ...
        await waitFor(() => {
            expect(mockFn).toHaveBeenCalledWith(expect.objectContaining({ priority: "High" }));
        });
    });
    it("E2E-01c — selecting a status filter calls getTickets with the status param", async () => {
        ...
        await user.click(screen.getByRole("button", { name: /apply filters/i }));
        await waitFor(() => {
            expect(mockFn).toHaveBeenCalledWith(expect.objectContaining({ status: "Resolved" }));
        });
    });
    it("E2E-01d — selecting a category filter calls getTickets with the categoryId param", async () => {
        ...
        await user.click(screen.getByRole("button", { name: /apply filters/i }));
        await waitFor(() => {
            expect(mockFn).toHaveBeenCalledWith(expect.objectContaining({ categoryId: 1 }));
        });
    });
    it("E2E-01e — clicking Next page calls getTickets with page 2", async () => {
        ...
        mockFn.mockResolvedValue({
            data: Array.from({ length: 10 }, (_, i) => makeTicket({ id: i + 1, ticketNumber: `TKT-${String(i + 1).padStart(4, "0")}` })),
    ...
    it("E2E-01f — Reset button clears all filters and reloads with empty params", async () => {
        ...
        await user.selectOptions(screen.getByRole("combobox", { name: /filter by priority/i }), "Low");
        await user.click(screen.getByRole("button", { name: /apply filters/i }));
        await waitFor(() => {
            expect(mockFn).toHaveBeenCalledWith(expect.objectContaining({ priority: "Low" }));
        });
        // Now reset
...
describe("API error handling — getTickets rejects", () => {
    it("displays the error banner and hides ticket content on a generic Network Error", async () => {
        ...
        expect(screen.getByRole("alert")).toHaveTextContent(/unable to load tickets/i);
