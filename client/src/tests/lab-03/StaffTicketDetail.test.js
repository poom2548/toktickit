...
const wrapper = ({ children }) => (_jsx(AuthProvider, { children: _jsx(MemoryRouter, { initialEntries: ['/staff/tickets/t1'], children: _jsx(Routes, { children: _jsx(Route, { path: "/staff/tickets/:id", element: children }) }) }) }));
describe('StaffTicketDetailPage', () => {
    beforeEach(() => {
        global.fetch = vi.fn();
    });
    it('renders all ticket detail sections', async () => {
        global.fetch.mockResolvedValue({ ok: true, status: 200, json: async () => mockTicket });
        render(_jsx(StaffTicketDetailPage, {}), { wrapper });
        await waitFor(() => {
    ...
    it('shows resolved flag indicator when problemAppearsResolved is true', async () => {
        ...
        global.fetch.mockResolvedValue({ ok: true, status: 200, json: async () => resolvedFlagTicket });
        render(_jsx(StaffTicketDetailPage, {}), { wrapper });
        await waitFor(() => {
