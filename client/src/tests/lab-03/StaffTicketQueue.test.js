...
const renderQueue = () => render(_jsx(MemoryRouter, { children: _jsx(StaffTicketQueuePage, {}) }));
...
it('renders table with all required columns on desktop', async () => {
    global.fetch = vi.fn(() => Promise.resolve(mockSuccessResponse));
    renderQueue();
    await waitFor(() => {
...
it('search input calls API with search param after debounce', async () => {
    const mockFetch = vi.fn(() => Promise.resolve(mockSuccessResponse));
    ...
    await waitFor(() => {
        ...
        const searchCall = calls.find(call => String(call[0]).includes('search=vpn'));
