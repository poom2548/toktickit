...
beforeEach(() => {
    vi.clearAllMocks();
    api.getTicketById.mockResolvedValue(mockTicket);
});
describe("TicketDetailPage", () => {
    it("LD1 & LD2 — Shows spinner while loading, then displays ticket details", async () => {
        // Delay resolution to see spinner
        api.getTicketById.mockImplementation(() => new Promise(res => setTimeout(() => res(mockTicket), 10)));
        render(_jsx(TicketDetailPage, { ticketId: 1, onBack: mockOnBack }));
    ...
    it("A1 — Upload valid file updates the attachment list", async () => {
        ...
        api.uploadAttachment.mockResolvedValue(newAttachment);
    ...
    it("S1, S2 — Soft-remove flow", async () => {
        ...
        api.removeAttachment.mockResolvedValue();
        render(_jsx(TicketDetailPage, { ticketId: 1, onBack: mockOnBack }));
        await waitFor(() => expect(screen.getByText("test.pdf")).toBeInTheDocument());
    ...
    it("G1 — Redirects if requester changes", async () => {
        const { rerender } = render(_jsx(TicketDetailPage, { ticketId: 1, onBack: mockOnBack }));
        await waitFor(() => expect(screen.getByDisplayValue("TKT-0001")).toBeInTheDocument());
        // Rerender with a different requester
        const newRequester = { id: 2, name: "Other User", email: "other@example.com" };
        rerender(_jsx(TicketDetailPage, { ticketId: 1, onBack: mockOnBack }));
        expect(mockOnBack).toHaveBeenCalled();
    });
    it("E1 — 403 shows not authorized alert", async () => {
        api.getTicketById.mockRejectedValue({ status: 403 });
        render(_jsx(TicketDetailPage, { ticketId: 1, onBack: mockOnBack }));
        await waitFor(() => {
    ...
    it("E2 — 404 shows not found alert", async () => {
        api.getTicketById.mockRejectedValue({ status: 404 });
        render(_jsx(TicketDetailPage, { ticketId: 1, onBack: mockOnBack }));
        await waitFor(() => {
